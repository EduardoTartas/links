...
const profilePicture =
  document.querySelector<HTMLImageElement>("#profilePicture")!;
...
const icons = document.querySelectorAll<SVGElement>(".icon");
...
if (data) {
  ...
  if (data.background) {
    const image = data.background.find(
      (item: { image?: string }) => "image" in item
    )?.image;
    const color = data.background.find(
      (item: { color?: string }) => "color" in item
    )?.color;
  ...
  //estilo do link
  if (data.linkStyle) {
    const defaultColor = data.linkStyle.find(
      (item: { defaultColor?: string }) => "defaultColor" in item
    )?.defaultColor;
    const hoverColor = data.linkStyle.find(
      (item: { hoverColor?: string }) => "hoverColor" in item
    )?.hoverColor;
    const linkColor = data.linkStyle.find(
      (item: { linkColor?: string }) => "linkColor" in item
    )?.linkColor;
    const borderRadius = data.linkStyle.find(
      (item: { borderRadius?: string }) => "borderRadius" in item
    )?.borderRadius;
